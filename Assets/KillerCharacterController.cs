...
public class KillerCharacterController : MonoBehaviour
{
    ...
    void HandleInteractions() {
        ...
        if (m_InputHandler.GetInteractInputDown()) {
            ...
            float killDistance = 3f;
            Collider[] objectsInBounds = Physics.OverlapSphere(transform.position, killDistance);
            foreach(Collider gameObject in objectsInBounds) {
                
                Actor actor = gameObject.GetComponent<Actor>();
                DeadBody deadBody = gameObject.GetComponent<DeadBody>();

                if (actor != null && actor.affiliation != 2) {
                    Debug.LogError("entity destroyed");

                    gameObject.gameObject.SetActive(false);
                    
                    // spawn a dead body here
                    Instantiate(deadbody, 
                    new Vector3(gameObject.gameObject.transform.position.x, gameObject.gameObject.transform.position.y - 1f, gameObject.gameObject.transform.position.z), 
                    Quaternion.identity);
                    
                    return; // only kill one person
